...
class SessionManager:
    ...
    async def load_session_from_db(self, session_schedule_id: str):
        ...
        try:
            ...
            # إعادة بناء البيانات
            embeddings_list, index_to_code, student_codes = await self.load_embeddings(students)
            ...
            for student_code in student_codes:
                ...
                if student_code in students_by_code:
                    student_map[student_code] = students_by_code[student_code].student_name
                    logger.info(f"✅ Using from session: {student_code} -> {students_by_code[student_code].student_name}")
                else:
                    # Fallback to collection
                    student_doc = await collection.find_one({"student_code": student_code})
                    if student_doc:
                        db_name = (
                            student_doc.get("student_name") or
                            student_doc.get("name") or
                            student_doc.get("full_name") or
                            student_code
                        )
                        student_map[student_code] = db_name
                        logger.info(f"✅ Using from collection: {student_code} -> {db_name}")
                    else:
    ...
    async def load_students(self, students: List[StudentSession]) -> Dict[str, str]:
        ...
        for student in students:
            ...
            if not existing_student:
                logger.warning(f"Student {student.student_code} not found in database, using request data")
                student_map[student.student_code] = student.student_name
                continue

            # 🔥 FIX: Try multiple field names for student name
            db_student_name = (
                existing_student.get("student_name") or
                existing_student.get("name") or
                existing_student.get("full_name") or
                student.student_name or
                student.student_code
            )
            
            student_map[student.student_code] = db_student_name
            logger.info(f"✅ Loaded student: {student.student_code} -> {db_student_name} (keys: {list(existing_student.keys())})")
    ...
    async def load_embeddings(
        ...
        if not embeddings_list:
            raise HTTPException(
                status_code=400,
                detail="No valid embeddings found"
            )
    ...
    async def get_faiss_index(self, session_schedule_id: str):
        ...
        logger.warning(f"FAISS index for {session_schedule_id} NOT in memory, loading from DB...")
    ...
    async def get_session_data(self, session_schedule_id: str):
        ...
        # ثانياً: حاول تحميل من MongoDB
        logger.warning(f"Session data for {session_schedule_id} NOT in memory, loading from DB...")
        ...
        # لم نستطع إيجاد البيانات
        logger.error(f"Session data for {session_schedule_id} not found in memory or DB")
        raise HTTPException(status_code=404, detail="Session not found in memory or database")
